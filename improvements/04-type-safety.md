...
- [ ] Zero `as X` type assertions in `src/` outside of `*.test.ts` and a single documented boundary module (casts at the untrusted-XML boundary are replaced by narrowing functions or schema parsing). _(Not started — no `src/rss/narrow.ts` boundary module yet; `as` casts remain scattered across `rss-feed.ts` and the mapping modules.)_
