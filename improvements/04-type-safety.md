...
- [x] Zero `as X` type assertions in `src/` outside of `*.test.ts` and a single documented boundary module (casts at the untrusted-XML boundary are replaced by narrowing functions or schema parsing). _(`src/rss/narrow.ts` created with `textOf` (extracts strings from plain values or `{ '#text': ... }` wrappers) and `recordOf` (narrows `unknown` to `Record<string, unknown>`). All XML-boundary `as` casts in `rss-feed.ts` replaced by `textOf`/`recordOf` or type predicates. Mapping module casts eliminated: `isValidTextRole` changed to a proper type predicate (`role is TextType`), inline `as TextComponent`/`as TikTokComponent` replaced by typed locals, `as ElementNode` casts replaced with `.find((n): n is ElementNode => ...)` type-predicate filters, `.pop() as string` replaced by `.pop()!`, `.map((n) => toImage(n as ElementNode))` replaced by type-predicate filter, `fromFigure(node) as ImageComponent` replaced by `isImageComponent(component)` guard with fallback, and the video/audio filter in `fromFigure` rewritten as a type-predicate filter eliminating `components.pop() as VideoComponent | AudioComponent`. The 3 `mapping as XxxMapping` casts in `mapping.ts` were eliminated by checklist item 5's discriminated-union refactor. Remaining `as` in `src/`: (a) `narrow.ts` itself (the documented boundary), (b) two structural/initialization casts in `rss-feed.ts` (`{} as unknown as ParsedXml` initialization fallback, iterator-check cast), (c) DOM-boundary casts in `parser.ts`/`html-mapper.ts` (different layer, not XML), (d) standard type-guard pattern (`object as Record<string, unknown>`) in `component.ts` is\* guards, (e) error-catch cast in `mapping.embeds.ts`. Full test suite (965 passed, 6 skipped) green.)_
...
- [x] Public API types are explicit exports (no leaking of internal helper types); `Readonly`/`readonly` modifiers applied to public-facing arrays/objects that consumers must not mutate. _(`src/index.ts` rewritten with explicit named exports — 8 `export *` collapsed into deliberate per-name exports grouped by domain (RSS, error model, network, component types, recipe schema, HTML mapper, mapping/params). Internal helpers removed from the public surface: all `*Schema` Zod exports from `component.ts`/`recipe-schema.ts`/`feed-issue.ts`; `feedIssue`/`errorIssue`/`warningIssue` factory functions; `reduceComponents`/`fromNode`/`getRootElement`/`reduceEmptyTextNode`/`filterEmptyTextNode` pipeline internals; `processTextLinks`/`isEmpty`/`mapLivePost`/`toCustom` utilities; `textTags`/`textTagsSet`/`mappingTagsSet`/`MAX_TEXT` constants. Public surface preserved: `RSSFeed`, `buildItem`/`BuildItemContext`, all RSS types, `replaceErrors`, `FeedIssue*` types + `FeedIssueCodes`, `FetchOptions`/`getHtmlContent`/`getRecipeFromUrl`, all `Component*` types + `is*` guards, all recipe schema types, `HTMLMapper`/`splitParagraphImages`, all Mapping/Filter types, `isValidParams`/`validateParams`/`isValidMapping`. `readonly` arrays added to `Component.errors`/`.warnings` (base type — all component builders already used local arrays, no post-creation mutation), `Item.errors`/`.warnings`/`.enclosure`/`.mediaGroup`/`.mediaContent`/`.components`/`.category`, `Enclosure.errors`/`.warnings`, `MediaGroup.errors`/`.warnings`/`.mediaContent`, `MediaContent.errors`/`.warnings`. `Channel`/`RSS` arrays left mutable — `validate()` and `build()` accumulate into them with `.push()` and direct reset (`= []`); making those readonly would require restructuring the validate/build pipeline (deferred). Fallout fixed: `buildItem` now passes the local mutable `errors`/`warnings` arrays to `buildCanvasflowFlags`/`buildThumbnail` (instead of `response.errors`/`response.warnings`); test `hasMessage` helpers widened to `readonly FeedIssue[]`; integration-test `.push()` calls replaced with spread-reassign; `.pop()` on `mediaContent` replaced with index access. `MutableItem` type alias (`DeepMutable<Item>`, recursively strips `readonly` from all array types) and `clone(item: Item): MutableItem` utility function added to `src/rss/rss-types.ts` and exported from the package root — consumers can call `clone(item)` to obtain a fully mutable deep-copy of any built `Item` for post-processing (`.push()`, `.splice()`, etc.). `tsc --noEmit` clean, 965 tests pass.)_
- [x] `getMappingComponent` returns a discriminated union; downstream casts in `fromNode` removed; `satisfies` on mapping tables _(`MappingComponentResponse` changed from a flat `interface` to a discriminated union type with one arm per component kind — `'columns'` → `ColumnsMapping`, `'live_container'` → `LiveContainerMapping`, `'gallery'` → `GalleryMapping`, `'recipe'` → `RecipeMapping`, `'container'` → `ContainerMapping`, `'custom'` → `CustomMapping`, `TextType` → `TextMapping`, `undefined` → `undefined`. `getMappingComponent` rewritten to switch on `mapping.component`, so TypeScript narrows the `mapping` value in each case arm. Call site in `fromNode` changed from destructuring to using the `mappingResult` object (`mappingResult.mapping`, `mappingResult.mappedComponent`, etc.) — property access preserves the discriminant relationship, so TypeScript narrows `mappingResult.mapping` to `ColumnsMapping`/`LiveContainerMapping`/`GalleryMapping` after each `if (mappingResult.mappedComponent === 'columns')` check, eliminating all 3 casts with zero new casts. `TEXT_TAG_MAPPING` annotated with `: Record<string, TextType>` and validated with `satisfies Record<string, TextType>` — `satisfies` validates values at the expression level while the annotation keeps the variable indexable by `string`. 965 tests pass, `tsc --noEmit` clean.)_
- [x] The public type surface is snapshotted (api-extractor report or `expect-type` tests) so accidental breaking changes fail CI. _(`src/__tests__/api-surface.test.ts` added — `expectTypeOf` assertions (using Vitest's built-in `expectTypeOf` re-exported from `vite-plus/test`) that encode the library's public type contracts: `validate()` returns `Promise<void>`, `build()` returns `Promise<RSS>`, every `Item` array field is `readonly`, `Enclosure`/`MediaContent` arrays are `readonly`, `MutableItem` arrays are mutable, `clone` parameter/return types, `FeedIssue` field types, `replaceErrors` parameter types, `HTMLMapper.toComponents` signature, every `is*` type guard predicate, `isValidParams`/`validateParams`/`isValidMapping` signatures, network utility parameter types, `buildItem`, and an import-existence guard that references every named type export in a union type — a removed or renamed type causes a compile error before tests run. Note: type-level conditional checks (`T extends U ? true : false`) are used for assertions involving `FeedIssue[]` / `RSS` — the large `FeedIssueCode` union (37+ literals) exceeds what `toEqualTypeOf`/`toMatchTypeOf` can represent internally without a TS2344 false positive. 1004 tests pass, 6 skipped.)_
- [x] `Component` narrowing relies on the `is*` guards everywhere (no `as SomethingComponent` in mapping modules). _(Verified: `grep -rn "as [A-Z][a-zA-Z]*Component" src/ --include="*.ts"` returns nothing outside test files. All pre-existing `as XxxComponent` casts in the mapping modules were eliminated in checklist items 1 and 5: `fromFigure`'s `as ImageComponent` replaced by `isImageComponent()` guard, video/audio filter replaced by type-predicate filter, and the 3 `mapping as XxxMapping` casts replaced by the discriminated-union refactor. No new casts introduced.)_
- [x] JSDoc on public API is accurate and `@param` types are kept as documented intent; descriptions added per function. _(Intentionally kept: `@param {type}` annotations are preserved so maintainers can attach per-parameter descriptions alongside the type. No action needed.)_
