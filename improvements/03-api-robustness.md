...
- [ ] The error model is typed: consumers get structured errors (code + message + path), not only interpolated strings; `rss.errors` is no longer a mixed `string`/`unknown` array. _(Partially done, scoped deliberately: `FeedIssue`/`FeedIssueCode` are now defined in `rss-types.ts` and used at the `params`/`root` Zod-validation boundary — `RSSFeed.validateParams` returns `FeedIssue[]` instead of raw `ZodIssue[]` blobs, tightening `RSS.errors` from `Array<unknown>` to `Array<string | FeedIssue>`. The rest of `errors`/`warnings` — RSS/channel/item level, and every `errors: string[]` on `Enclosure`/`MediaContent`/`MediaGroup`/mapping components — is still plain strings. Full migration was assessed as a major-version, high-blast-radius change touching most existing test assertions; deferred by explicit choice rather than done partially without a decision record. Revisit alongside ADR-0008's suggestion to bundle it with the sync conversion in one major.)_
...
- [ ] `docs/wiki/API-Reference.md` documents the contract: what throws (nothing), what errors/warnings mean, and the validate/build lifecycle. _(Not done yet — only the `validateParams` signature line was updated for the `FeedIssue` return type. Full contract documentation + `CHANGELOG.md` migration guide is the remaining item.)_
