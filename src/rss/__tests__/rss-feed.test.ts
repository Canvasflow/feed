...
import { replaceErrors, RSSFeed } from '../rss-feed';
...
describe('Recipe — offline (injected fetch, local fixtures)', () => {
  const httpPath = path.join(__dirname, '..', '..', 'support', 'http');

  function fixtureStub(filename: string): typeof fetch {
    return async () => {
      const body = readFileSync(path.join(httpPath, filename), 'utf-8');
      return {
        ok: true,
        status: 200,
        text: async () => body,
  ...
  test(
    'returns a recipe object from a page with LD+JSON Recipe',
    { tags: ['unit', 'recipe'] },
    async () => {
      const recipe = await getRecipeFromUrl(
        'https://www.saga.co.uk/magazine/recipes/easter-simnel-cake',
        {
          fetch: fixtureStub('saga-easter-simnel-cake.html'),
        }
      );
  ...
  test(
    'returns the exact recipe object when a valid recipe is found',
    { tags: ['unit', 'recipe'] },
    async () => {
      const recipe = await getRecipeFromUrl(
        'https://www.veganfoodandliving.com/vegan-recipes/vegan-steamed-jam-suet-sponge-pudding',
        { fetch: fixtureStub('veganfoodandliving-steamed-jam.html') }
      );
  ...
  test(
    'returns null when no recipe LD+JSON is present',
    { tags: ['unit', 'recipe'] },
    async () => {
      const recipe = await getRecipeFromUrl(
        'https://www.veganfoodandliving.com/news/wagamama-vegatsu-vegan-options-menu-cuts/',
        { fetch: fixtureStub('veganfoodandliving-news.html') }
      );
...
describe('RSSFeed build — item edge cases', () => {
