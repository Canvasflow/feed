...
describe('Node helpers', () => {
  ...
  test(
    ...
    () => {
      const tree: Node[] = [
        {
          ...
          children: [
            { type: 'text', content: 'Caption text' },
            {
              type: 'element',
              tagName: 'span',
              children: [{ type: 'text', content: 'credit' }],
              attributes: [{ key: 'class', value: 'credit' }],
            },
          ],
        },
      ];
      const result = tree.reduce(
        removeDescendants(
          (n) =>
            n.type === 'element' &&
            (
              n as { attributes?: { key: string; value: string }[] }
            ).attributes?.some(
              (a) => a.key === 'class' && a.value === 'credit'
            ) === true
        ),
        []
      );
      ...
      expect(figcaption.children[0]).toMatchObject({
        type: 'text',
